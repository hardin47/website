# Introduction^[Much of this content was inspired by great educators who provide open source materials for educational use.  Many thanks to Mine Çetinkaya-Rundel (Duke), Ben Baumer (Smith), Brianna Heggeseth (Macalester), Leslie Myint (Macalester), Paul Roback (St Olaf) for sharing their materials.] {#intro}
...
```{r fig.cap = "A schematic of the typical workflow used in data analysis.  Most statistics classes focus only on the left side.  We will work to address all aspects (including those on the right side). [@Baumer15]", fig.width = 7, fig.align='center', echo=FALSE}
knitr::include_graphics("figs/AnalysisProcessBaumer.jpg")
```


```{r fig.cap = "[Stitch Fix Algorithms Tour](http://algorithms-tour.stitchfix.com/)", fig.width = 7, fig.align='center', echo=FALSE}
...
```{r fig.width = 7, fig.align='center', echo=FALSE}
knitr::include_graphics("figs/twitterscatter.jpg")
knitr::include_graphics("figs/twitterreg.jpg")
```
...
1. What steps are necessary to reproduce this study? Be as specific as you can! Try to list the sub-tasks that you would have to perform.
...
```{r fig.width = 7, fig.align='center', echo=FALSE}
knitr::include_graphics("figs/twitter_rojas.jpg")
knitr::include_graphics("figs/twitter_linkins.jpg")
```
