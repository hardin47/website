...
knitr::include_graphics("figs/BSlogic.png")
```
...
| Ideal Real World |   | Bootstrap World   |
|---------------|-------|-------------------|
...
knitr::include_graphics(c("figs/BShesterberg1.png",
                          "figs/BShesterberg2.png",
                          "figs/BShesterberg3.png"))
```
...
1. Take $B_1$ bootstrap samples, and for each sample $\underline{X}^{*b}$ compute the bootstrap estimate $\hat{\theta}^*(b)$.
...
4. Calculate the CI as in @eq-BSt.
...
* The resulting intervals will typically not (necessarily) be symmetric (that is $\hat{t}^*_\alpha \ne - \hat{t}^*_{1-\alpha}).$  This is part of the improvement over $z$ or $t$ intervals.
...
Why does it work? It isn't immediately obvious that the interval above will capture the true parameter, $\theta$, at a rate or 95%.  Consider a skewed sampling distribution.  If the observed $\hat{\theta}$ comes from the long tail, is it obvious that the short tail side of the CI will get up to the true parameter value at the correct rate?  (@hall1992 (and earlier papers) refers to these as Efron's "backwards" intervals.) Or, if the sampling distribution is biased, the percentiles of the bootstrap interval won't capture the parameter with the correct rate.
...
Going back to @eq-phidist indicates that $\hat{\phi} + \sigma z_{1-\alpha} = F^{-1}_{\hat{\phi}^*}(1-\alpha)$ (because $\hat{\phi} ^* \sim N(\hat{\phi}, \sigma^2)$).  Further, since $g$ is monotonically increasing, $F^{-1}_{\hat{\phi}^*}(1-\alpha) = g(F^{-1}_{\hat{\theta}^*}(1-\alpha)).$
Substituting in @eq-phiint, gives the percentile interval for $\theta$,
...
#### Bias {-}
The percentile interval is not, however, perfect.  If the statistic is a *biased* estimator of the parameter, there will not exist a transformation such that the distribution is centered around the correct function of the parameter.  Formally,  if
\begin{align}
...
| BS-t 	| No 	| No 	| No 	| $2^{nd}$ order 	| No 	| computer intensive 	|
...
heroin |>
  ...
  ggplot(aes(x=times)) +
  geom_histogram(bins=30) + 
  labs(title = "one bootstrap resample",
       y = "")
```
...
ggplot(boot_stats, aes(x=boot_tr_mean)) + 
  geom_histogram(bins=20) + 
  labs(title = "dist of trimmed mean",
       y = "") +
  xlim(c(300, 475)) +
  geom_vline(aes(xintercept = mean(boot_med))) 
```
...
p1 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_tr_mean)) + 
  labs(x = "boostrapped \ntrimmed means")
p2 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_se_tr_mean)) + 
  labs(x = "boostrapped \nSEs (trim mean)")
p3 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_t_tr_mean)) + 
  labs(x = "boostrapped \nt-like-stats (trim mean)")
...
p4 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_med)) + 
  labs(x = "boostrapped \nmedians")
p5 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_se_med)) + 
  labs(x = "boostrapped \nSEs (median)")
p6 <- boot_t_stats |>
  ggplot() + 
  geom_histogram(aes(x =boot_t_med)) + 
  labs(x = "boostrapped \nt-like-stats (median)")
