...
library(png)
library(grid)
img1 <- readPNG("figs/genderbias2a.png")
img2 <- readPNG("figs/genderbias2b.png")
 grid.raster(img1, x=0.5, y=.75); grid.raster(img2, x=.5, y=.1)
```


```{r fig.width=15, fig.align="center",  echo=FALSE}
img3 <- readPNG("figs/genderbias.png")
 grid.raster(img3)
```

```{r fig.width=15, fig.align="center", echo=FALSE}
img4 <- readPNG("figs/genderbias3a.png")
 grid.raster(img4)
```


```{r fig.width=15, fig.align="center", echo=FALSE}
img5 <- readPNG("figs/genderbias3b.png")
 grid.raster(img5)
```
...
macnell %>% group_by(tagender) %>%
  ...
  select(overall, tagender, taidgender, permTAID) %>% head(15)
') %>%
  hlt_fixed("group_by(tagender)")
```
...
```{r fig.width=15, fig.align="center", echo=FALSE}
img6 <- readPNG("figs/genderbias3c.png")
 grid.raster(img6)
```
