...
```{r echo = FALSE, out.width = "60%"}
knitr::include_graphics("figs/garbage.png")
```
...
```{r fig.cap = "Great interactive viz at http://www.r2d3.us/visual-intro-to-machine-learning-part-2/", out.width='100%', fig.align='center', echo=FALSE}
knitr::include_graphics("figs/bias-var-cv.png")
```
...
knitr::include_graphics("figs/testtrain.png")
```
...
```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide2.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide3.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide4.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide5.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide6.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide7.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide8.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide9.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide10.png")
```

$$\bigg\Downarrow$$

```{r echo = FALSE, fig.cap = "Image credit: Alison Hill", out.width = "20%"}
knitr::include_graphics("figs/CV/Slide11.png")
```
...
knitr::include_graphics("figs/modelbuild1.png")
```

```{r echo = FALSE, fig.cap = "Image credit: https://www.tmwr.org/"}
knitr::include_graphics("figs/modelbuild2.png")
```


```{r echo = FALSE, fig.cap = "Image credit: https://www.tmwr.org/", fig.align='center'}
knitr::include_graphics("figs/modelbuild3.png")
```
...
knitr::include_graphics("figs/sfnyc.png")
```
...
```{r fig.cap = "https://www.nytimes.com/interactive/2019/08/08/opinion/sunday/party-polarization-quiz.html Quiz: Let Us Predict Whether You're a Democrat or a Republican NYT, Aug 8, 2019.  Note that race is the first and dominant node, followed by religion.", out.width='100%', fig.align='center', echo=FALSE}
knitr::include_graphics("figs/partyaffiliation.png")
```

```{r include = FALSE, fig.cap = "http://graphics8.nytimes.com/images/2008/04/16/us/0416-nat-subOBAMA.jpg Best information was whether or not the county was more than 20 percent black.   Then each successive node is split again on the best possible informative variable.  Note that the leaves on the tree are reasonably homogenous. NYT, April 16, 2008.", out.width='100%', fig.align='center', echo=FALSE}
...
* *At each split*, select `mtry` variables and determine the best split using only the `mtry` predictors.
Typically $m = \sqrt{p}$ or $\log_2 p$, where $p$ is the number of features.  Random Forests are not overly sensitive to the value of `mtry`.  [splits are chosen as with trees:   according to either squared error loss or gini index / cross entropy / classification error.]
* Do *not* prune the tree.  Save the tree as is!

3. Repeat steps 1-2 for many many trees.
4. For each tree grown on a bootstrap sample, predict the OOB samples.  For each tree grown, $\sim 1/3$ of the training samples won't be in the bootstrap sample -- those are called out of bootstrap (OOB) samples.  OOB samples can be used as *test* data to estimate the error rate of the tree.
...
* generally, models do not overfit the data and CV is not needed.  However, CV can be used to fit the tuning parameters (`mtry`, node size, max number of nodes, etc.).   
...
* small `mtry` (e.g., `mtry = 1`)
   - increased bias:  when `mtry` is small, each tree has fewer options for the best splitting variable. So, the individual trees might miss important features.
   - reduced variance: a small `mtry` increases the randomness and diversity between the individual trees in the forest. Aggregating the decorrelated trees reduces the overall variance of the forest predictions.
   
* big `mtry` (e.g., when `mtry = p` where `p` is the number of variables)
   - reduced bias: when `mtry` is large, individual trees have a chance to use the important variables which reduces the bias of individual trees.
   - increased variance: a large `mtry` means that the individual trees are more similar to each other, so aggregating over all the trees doesn't substantially reduce the variance of the entire forest prediction.
...
Let ${\bf x} = (x_1, x_2, \ldots, x_p)^t$ and ${\bf y} = (y_1, y_2, \ldots, y_p)^t$ be two vectors which live in $R^p$ and have an angle between them of $\theta$.  Then their dot product is defined as:
\begin{align}
{\bf x} \cdot {\bf y} = {\bf x}^t {\bf y} = \sum_{i=1}^p x_i y_i = ||{\bf x}|| \cdot ||{\bf y}|| \cdot cos(\theta)
\end{align}
...
$$\begin{align}
{\bf w} \cdot {\bf x}_+ + b &\geq 1
\end{align}$$ {#eq-posconstr}

$$
\begin{align}{\bf w} \cdot {\bf x}_- + b &\leq -1 
\end{align}$$ {#eq-negconstr}
...
(Multiplying through by -1 on equation (@eq-negconstr) switches the signs, and both equation (@eq-posconstr) and (@eq-negconstr) end up as the same for both types of points.)
...
If <center><p style="color:red">the data can be transformed into a new space where the data **are** linearly separable...</p></center>   If we can transform the data into a different space (where they are linearly separable), then we can do the support vector work in the new space! That is, consider the function $\phi$ such that our new space consists of vectors $\phi({\bf x})$.
...
```{r fig.cap = "Extremely complicated decision boundary", out.width='45%', fig.align='center', echo=FALSE, fig.show = 'hold'}
knitr::include_graphics(c("figs/SVMEx1.jpg", "figs/SVMEx1g100.jpg"))
```
...
```{r fig.cap = "Simple decision boundary", out.width='60%', fig.align='center', echo=FALSE}
knitr::include_graphics("figs/SVMEx2.jpg")
```

```{r fig.cap = "Simple decision boundary -- reasonable gamma", out.width='45%', fig.align='center', echo=FALSE, fig.show = 'hold'}
knitr::include_graphics(c("figs/SVMEx2g1.jpg", "figs/SVMEx2g10.jpg"))
```

```{r fig.cap = "Simple decision boundary -- gamma too big", out.width='45%', fig.align='center', echo=FALSE, fig.show = 'hold'}
knitr::include_graphics(c("figs/SVMEx2g100.jpg", "figs/SVMEx2g1000.jpg"))
```
