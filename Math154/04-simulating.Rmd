...
>So what happens if we don't allow our classifier to see group membership, and just train one classifier on the whole student body? The gap in false negative rates between the two populations balloons to `r error.rates[2,4]*100 - error.rates[1,4]*100`%, and the overall error rate ticks up. This means if you are a qualified member of the Red population, you are substantially more likely to be mistakenly rejected by our classifier than if you are a qualified member of the Blue population.
...
```{r fig.cap = "Weibull PDF by Calimo - Own work, after Philip Leitch.. Licensed under CC BY-SA 3.0 via Commons", fig.width = 7, fig.align='center', echo=FALSE}
knitr::include_graphics("figs/Weibull_PDF.png")
knitr::include_graphics("figs/Weibull_CDF.png")
```
