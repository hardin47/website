...
Here is an algorithm for finding words that represent $K$ topics (where $K$ is chosen in advance).
...
2. Each of the $K$ topics now has a set of words associated with it (albeit, that set of words is meaningless). To improve on the set of words associated with each topic.  For each word $w$ in document $d$:
    a. For each topic $t$, compute two things: 
        * p(topic $t$ $|$ document $d$) = the proportion of words in document $d$ that are currently assigned to topic $t$.  Actually, we use:
        $$P(z_i = t | z_{-i}, w_i, d_i) = \frac{\# \mbox{ words in $d$ with $t$} + \alpha}{\# \mbox{ words in $d$ with any topic} + k \cdot \alpha}$$
        $z_i$ is the topic of the $i^{th}$ word; $z_{-i}$ is the topic of all the words except the $i^{th}$ word; $w_i$ is the $i^{th}$ word; $d_i$ is the document for the $i^{th}$ word; and $\alpha$ is a prior parameter.
