...
* $d_P$ can be zero even if ${\bf x} \ne {\bf y}$ (happens when ${\bf x}$ is a linear combination of ${\bf y}$).
...
* It is a (mathematical) distance metric.
...
Now consider a new observation, $x^* = x_3 = 5$. 
Using **complete** linkage, $x^*$ is closer to $\{x_4, x_5 \}$ (distance = 5) than to $\{x_1, x_2, x_3, x_6 \}$ (distance = 8).
...
We can use the model to predict in which cluster each penguin belongs. Note that the predicted cluster 
...
`stats` engine lets us `tidy()` and `glance()` and `augment()`. With the `ClusterR` engine, we can only `augment()` (see below).
...
The function `augment()` works at the per-observation level:

```{r}
kmean_fit |> augment(new_data = penguins |> drop_na())
```
