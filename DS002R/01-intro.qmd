# Introduction^[Much of this content was inspired by great educators who provide open source materials for educational use.  Many thanks to Mine Çetinkaya-Rundel (Duke), Ben Baumer (Smith), Brianna Heggeseth (Macalester), Leslie Myint (Macalester), Paul Roback (St Olaf), and Ciaran Evans (Wake Forest) for sharing their materials.] {#sec-intro}
...
We will work through acquiring data (e.g., web scraping and using SQL), data exploration (e.g., data wrangling, text analysis), data visualization, data conclusions (e.g., iteration, permutation tests), and data communication (e.g., reproducible workflow).
...
R is freely available at http://www.r-project.org/ and is already installed on college computers. Additionally, installing Positron is required https://posit.co/downloads/.
...
tribble(
  ...
  gt::cols_width(`DS workflow` ~ pct(30), everything() ~ pct(35))
```
...
* <a href = "https://cs.pomona.edu/~ajc/" target = "_blank">Anthony (Tony) Clark</a> uses data to improve the safety and reliability of mobile robots.
...
* Literate programming $\rightarrow$ Quarto (via Positron)
...
* You must use both R and Positron software programs
* R does the programming
* Positron brings everything together
...
knitr::include_graphics("images/r_vs_positron.png")
```
...
* Then, in Positron also edit the README document with a different change.
    * Commit your changes
...
* What is the difference between R and Positron?
