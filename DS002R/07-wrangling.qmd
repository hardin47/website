...
1. cheatsheets: https://opensource.posit.co/resources/cheatsheets/?languages=R
2. tidyverse vignettes: https://cran.r-project.org/web/packages/tidyverse/vignettes/paper.html
...
Notice below that the value of `mean_delay` is different for the two queries.

##### Order 1: filter first, then mutate {-}
```{r}
...
flights |>
  ...
  select(year, month, day, dep_delay, mean_delay, origin, dest, carrier)
```

##### Order 2: mutate first, then filter {-}
```{r}
flights |>
  group_by(carrier) |>
  mutate(mean_delay = mean(dep_delay, na.rm = TRUE)) |>
...
Taken from the dplyr tutorial: <a href = "https://dplyr.tidyverse.org/" target = "_blank">https://dplyr.tidyverse.org/</a>
...
```{r}  
babynames |> group_by(sex) |> summarize(total = sum(n))
...
There are more complicated verbs which may be important for more sophisticated analyses.  See the Positron **dplyr** cheatsheet,  https://opensource.posit.co/resources/cheatsheets/data-transformation/.
...
navy_gs <- read_sheet(
  "https://docs.google.com/spreadsheets/d/1Ow6Cm4z-Z1Yybk3i352msulYCEDOUaOghmo9ALajyHo/edit#gid=1877566408",
...
navyWR <- navy |>
  select(-contains("tot")) |>
  filter(
    substr(pay.grade, 1, 5) != "TOTAL" &
      substr(pay.grade, 1, 5) != "GRAND"
  ) |>
  pivot_longer(-pay.grade, values_to = "numPeople", names_to = "status") |>
...
navyWR |>
  ...
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, size = rel(.5))
  )
```
...
gs4_deauth()
litF <- read_sheet(
  "https://docs.google.com/spreadsheets/d/1hDinTIRHQIaZg1RUn6Z_6mo12PtKwEPFIz_mJVF6P5I/pub?gid=0"
)

litF <- litF |>
  select(country = starts_with("Adult"), starts_with("1"), starts_with("2")) |>
  pivot_longer(-country, names_to = "year", values_to = "litRateF") |>
...
GDP <- GDP |>
  select(country = starts_with("Income"), starts_with("1"), starts_with("2")) |>
...
Mostly, the backslash will be used for functions we already know:
...
penguins_split |>
  map(\(mydata) lm(body_mass_g ~ flipper_length_mm, data = mydata)) |>
...
* What are the input and output of a data verb?

* What is the difference between `pivot_wider()` and `pivot_longer()`?

* Does `group_by()` do anything if it is the last verb in the chain? What does it do if it is followed by a `summarize()` verb?

* What are the differences across the different `join()` functions?

* What does `map()` do? What are the arguments of `map()`?
...
* Is it okay to just remove an observation if some of the information is missing? Why or why not?

* In what way does the pipe (`|>`) help with communication?
