...
**Color** (either) to what extent? how severely? Beware of red/green color blindness. Use color-vision-safe palettes like <a href = "https://ggplot2.tidyverse.org/reference/scale_viridis.html" target = "_blank">`viridis`</a> or < href = "https://malcolmbarrett.github.io/ggokabeito/" target = "_blank">`Okabe–Ito`</a>.
...
SAT |>
  ...
  labs(x = "state", y = "average SAT score") +
...
```{r, echo=FALSE}
ggplot(data = SwimRecords, aes(x = year, y = time, color = sex)) +
  geom_point() +
  geom_line() +
  labs(title = "100m Swimming World Records", y = "World Record Time (seconds)")
```
...
2. `ggplot2` cheat sheet: https://opensource.posit.co/resources/cheatsheets/data-visualization/
...
```{r, eval=FALSE}
ggplot(data = Births78, aes(x = date, y = births, color = "navy")) +
  geom_point() +
  labs(title = "US Births in 1978")
```
...
```{r}
ggplot(data = Births78, aes(x = date, y = births)) + # map x & y
...
```{r, eval=TRUE}
ggplot(data = Births78, aes(x = date, y = births, fill = wday)) +
  geom_area() +
  labs(title = "US Births in 1978")
```
...
```{r}
ggplot(data = HELPrct, aes(x = substance)) +
  geom_bar() +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
ggplot(data = HELPrct, aes(x = substance, fill = sex)) +
  geom_bar() +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
```{r, fig.height=1.6}
ggplot(data = HELPrct, aes(x = i1, color = substance, linetype = sex)) +
  geom_line(stat = "density") +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
```{r, fig.height=1.6}
ggplot(data = HELPrct, aes(x = i1, color = substance)) +
  geom_line(stat = "density") +
  facet_grid(. ~ sex) +
  labs(title = "HELP clinical trial at detoxification unit")
```
...
* Why is making a poor plot problematic?

* Who is responsible for poor communication when the data visualization doesn't convey the appropriate information (e.g., with the Challenger)?

* Why should you avoid encoding meaning (e.g., a variable) using color alone? Hint: use color-vision-safe palettes like <a href = "https://ggplot2.tidyverse.org/reference/scale_viridis.html" target = "_blank">`viridis`</a> or < href = "https://malcolmbarrett.github.io/ggokabeito/" target = "_blank">`Okabe–Ito`</a>. 
