...
* [**stringr** package](https://stringr.tidyverse.org/) A core package in the **tidyverse**. It is installed via `install.packages("tidyverse")` and also loaded via `library(tidyverse)`. Of course, you can also install or load it individually.
     ...
     * <a href = "https://opensource.posit.co/resources/cheatsheets/strings/" target = "_blank">**stringr** cheatsheet</a>
...
allCounts <- data.frame(
  Geneid = c(
    "CDS:b2743:pcm:L-isoaspartate_protein_carboxylmethyltransferase_type_II:cds2705:-:626:NC_000913.3",
...
```{r}
allCounts <- allCounts |>
  tidyr::separate(Geneid, c("feature", "rest"), sep = "[:]")
allCounts
```
...
igr <- igr |>
  tidyr::separate(GeneidBackup, c("Geneid1", "Geneid2"), sep = "[/]")
names(igr)
igr
```
...
igr <- igr |>
  tidyr::separate(start.gene, into = c("comma", "start.gene"), sep = "[,]") |>
  dplyr::select(-comma) |>
...
* How can we use regular expressions to check for name misspellings or other similar data cleaning tasks?

* Name one thing that you noticed about regular expressions in the course materials (either in class or reading the notes, etc.) where you thought to yourself "Oh, I'll have to be really careful about **that**."  Why would you need to be careful?
