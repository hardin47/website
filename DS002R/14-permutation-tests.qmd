...
num_exper <- 5000
help_random <- map_dbl(1:num_exper, random_choice, num_babies = 16)
...
help_random |>
  ...
  labs(
    x = "proportion of babies who chose the helper",
...
hsb2 |>
  group_by(schtyp) |>
  summarize(ave_math = mean(math), med_math = median(math))
```
...
hsb2 |>
  group_by(schtyp) |>
  summarize(ave_math = mean(math), med_math = median(math)) |>
  summarize(ave_diff = diff(ave_math), med_diff = diff(med_math))
```
...
perm_stats |>
  summarize(
    p_val_ave = mean(perm_ave_diff > obs_ave_diff),
...
### Stratified two-sample permutation test
...
macnell <- readr::read_csv(
  "https://raw.githubusercontent.com/statlab/permuter/master/data-raw/macnell.csv"
)
#library(permuter)
#data(macnell)
```
...
distr1 <- stratified_two_sample(
  response = macnell$overall,
...
macnell |>
  group_by(taidgender) |>
  summarize(
    means = mean(overall, na.rm = TRUE),
    vars = var(overall, na.rm = TRUE),
    ns = n()
  ) |>
