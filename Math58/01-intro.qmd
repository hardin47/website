...
* Use Quarto to run reproducible analyses that include all aspects of the data analysis.
...
```{r fig.cap = "Taken from [Modern Drive: An introduction to statistical and data sciences via R](https://ismayc.github.io/moderndiver-book/), by Ismay and Kim", out.width="80%", fig.align='center', echo=FALSE}
...
## Example: Friend or Foe {#sec-helper}
...
ggplot(null_help) + 
  geom_histogram(aes(x = stat), color = "white", bins = 13) + 
  geom_vline(xintercept = p_obs, color = "red", linewidth = 2) + 
  geom_vline(xintercept = 2*0.5 - p_obs, color = "red",
             linewidth = 2) + 
  labs(title = "Simulation based null distribution",
       x = "Possible values for the sample proportion when p = 0.5")

# calculate the actual p-value
null_help |> 
  summarize(p_value = 2*mean(stat >= p_obs))
```
...
```{r fig.cap = "Random Sample vs Randomized Experiment, taken from @sleuth", out.width="95%", fig.align='center', echo=FALSE}
#| label: fig-randSleuth
knitr::include_graphics("figs/randsampexp.png")
```

```{r fig.cap = "Random Sample vs Randomized Experiment, taken from https://askgoodquestions.blog/", out.width = "95%", fig.align='center', echo=FALSE}
#| label: fig-randgoodQ
...
4. Have you looked at @fig-randSleuth and @fig-randgoodQ? Do you understand the two figures?  Could you explain what their main message is to a friend? [Random sampling vs. Random allocation]
