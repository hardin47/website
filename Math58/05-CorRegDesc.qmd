# Correlation & Regression as Models {#sec-regdesc}
...
$s^2$ is the amount the response variable varies on its own.  $SSE(\mbox{least-squares})$ is the amount the response variable varies around the regression line (see @sec-slr).
...
###  Least Squares estimation of the regression line {#sec-ls}
...
###  Extrapolation {#sec-extrap}

In linear regression, extrapolation happens when the model is used to predict values that fall outside of the range of observations that created the model. Extrapolation can be problematic because there may be no reason to believe that the shape of the model continues beyond the given data.  See Figure @fig-extrap.

```{r}
#| label: fig-extrap
#| fig.cap: "Tweet by Ethan Anderson, March 4, 2026"
#| out.width: "80%"
#| fig.width: 9
#| fig.align: 'center'
#| echo: false
knitr::include_graphics("figs/extrapolate.png")
```
