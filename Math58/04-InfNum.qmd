...
We will talk about this model in more detail in Chapter \@ref(reginf) on Multiple Regression and inference, but for now notice that the `mdw_pippit` and `wren` are discernibly (significantly) different from the `hedge_sparrow` (p-values are small), but the `robin`, `tree_pippit` and `wagtail` are not significantly different from the `hedge_sparrow` (p-values are big). 
