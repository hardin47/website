...
Spend some time clicking through different datasets in the ICAM applet: [http://www.rossmanchance.com/applets/OneSample.html?showBoth=1](http://www.rossmanchance.com/applets/OneSample.html?showBoth=1)
...
### Example: The Ethan Allen^[Inv 2.4, Chance & Rossman, ISCAM]

> In October, 2005, a tour boat named the Ethan Allen capsized on Lake George in New York with 47 passengers aboard. In the inquiries that followed, it was suggested that the tour operators should have realized that the combined weight of so many passengers was likely to exceed the weight capacity of the boat. 

> Data from the Centers for Disease Control and Prevention indicate that weights of American adults in 2005 had a mean of 167 pounds and a standard deviation of 35 pounds. (To convey that these are population values, we will use Greek letters to represent their values, $\mu$ = 167 and $\sigma$ = 35.)

> Engineers estimated the maximum weight capacity of passengers that the Ethan Allen could accommodate to be 7500 pounds. If the tour boat company consistently accepted 47 passengers, we want to know whether (and how often) the weight of the 47 passengers will exceed the boat's capacity.

The shape of the data is unknown. They are unlikely to come from a symmetric normal distribution, but the CLT tells us that even if the data are skewed, the sampling distribution for the sample mean will be normal (bell shaped, symmetric). However, when we standardize by dividing by the SE we get a t statistic (distribution: $t_{n-1}$) instead of a Z statistic (distribution: N(0,1)). 

Note: when working with proportions, we also typically divided by the SE (instead of the SD)! However, the mathematics driving the t distribution apply only for numeric variables and not for binary variables. So t distributions are only relevant for standardized statistics like means and regression coefficients.

Assume that the CDC population represents the population from which we take random samples of size 47 for daily trips on the Ethan Allen. What is the probability that, on any given day, the weight of the passengers would be above the boat's capacity?

\begin{eqnarray*}
P(\mbox{total weight} > 7500) &=& P(\overline{X} > 159.57 | \mu = 167, \sigma = 35)\\ 
&=& P(Z > \frac{159.57 - 167}{35/\sqrt{47}}) \\
&=& P(Z > -1.46)\\
&=& 0.928
\end{eqnarray*}

```{r}
1 - mosaic::xpnorm(-1.46)
```

That is, on 92.8% of the days, the weight on the boat was over the boat's capacity.

n.b., we used `xpnorm()` instead of `xpt()` because we knew the population value of $\sigma$ (we didn't estimate it from the data).

## Inference for a single mean, $\mu$ {#sec-mean1inf}
...
ggplot(null_dist) + 
  geom_histogram(aes(x = stat), bins = 20, color = "white") + 
  geom_vline(xintercept = heart_obs, color = "red",
             linewidth = 2) +
  labs(title = "Simulation based null distribution",
       x = "Possible values for the difference in sample means")


null_dist |> 
  summarize(p_value = mean(stat >= heart_obs))
```
...
ggplot(boot_dist) + 
  geom_histogram(aes(x = stat), color = "white", bins = 20) + 
  geom_vline(xintercept = heart_ci$lower_ci, color = "blue") + 
  geom_vline(xintercept = heart_ci$upper_ci, color = "blue") +
...
Paired observations (e.g., a "before" and an "after") will be treated as a **single** measurement (i.e., the difference between the two measurements).  As the difference is a single measurement, the analysis tools will be identical to those in @sec-mean1inf, Inference for a Single Mean, $\mu$
...
* The normal probability plot (or dotplot or histogram) for each sample's responses is reasonably bell-shaped.
...
For simplicity, we will apply the same checks for a randomized experiment as well (with the last condition being met if the treatments are randomly assigned).
If the first two conditions are not met, then suitable transformations may be useful.
...
We will talk about this model in more detail in Chapter [-@sec-reginf] on Multiple Regression and inference, but for now notice that the `mdw_pippit` and `wren` are discernibly (significantly) different from the `hedge_sparrow` (p-values are small), but the `robin`, `tree_pippit` and `wagtail` are not significantly different from the `hedge_sparrow` (p-values are big). 
...
Let's say we were originally interested in whether the eggs hatched in meadow pipit and tree pipit nests were discernibly (significantly) different sizes, on average.  
...
As you might expect, if you have 10 groups, **all of which come from the same population**, you might wrongly conclude that some of the means are *discernibly* (significantly) different.  If you try pairwise comparisons on all 10 groups, you'll have ${10 \choose 2} = 45$ comparisons.  Out of the 45 CI, you'd expect 2.25 of them to not contain the true difference (of zero); equivalently, you'd expect 2.25 tests to reject the true $H_0$.  In an overall test of comparing all 10 groups simultaneously, you can't use size as a performance measure anymore.
...
We can use Bonferroni or the Tukey-Kramer method to adjust the FWER.  Other possibilities for determining discernibility (like the false discover rate (FDR) or q-values) also exist.
...
```{r}
teachers |>
  t.test(base ~ degree, alternative = "less", data = _)
```
...
ggplot(null_dist) + 
  geom_histogram(aes(x = stat), color = "white", bins = 25) + 
  geom_vline(xintercept = x_bar_base, color = "red", linewidth = 2) + 
  labs(title = "Simulation based null distribution",
       x = "Possible values for the difference in sample means")

# calculate the p-value
null_dist |> 
  summarize(p_value = mean(stat >= x_bar_base))
...
ggplot(null_dist) + 
  geom_histogram(aes(x = stat), bins = 20, color = "white") + 
  geom_vline(xintercept = diff_x_bar_base, color = "red",
             linewidth = 2) +
  labs(title = "Simulation based null distribution",
       x = "Possible values for the difference in sample means")
...
# calculate the p-value
null_dist |> 
  summarize(p_value = mean(stat >= diff_x_bar_base))
```
