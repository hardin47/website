...
### Using randomization test for inference on RR (tool: **infer**)
...
### Using randomization tests for inference on OR (tool: **infer**)
...
3. What is the test statistic (it is the same for both the randomization simulation and the chi-squared test with the mathematical model!!)?  Why do we need a complicated test statistic here and we didn't need one with $2 \times 2$ tables?
