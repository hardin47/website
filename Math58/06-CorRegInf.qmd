# Inference on Correlation & Regression {#sec-reginf}
...
### Inference for correlation [Not covered Spring 2026]
...
See the Cat Jumping^[@iscam, Inv 5.6 & 5.13] example below in @sec-cat2.
...
p-value &=& 2 \cdot P(t_{18-2} \leq -2.2849) = 2\cdot(pt(-2.2849,16)) = 0.036 \mbox{  (borderline discernible (significant))}
\end{eqnarray*}
...
As in section @sec-ls, the slope is calculated by using least squares.
...
Where $MSE = \frac{\sum_i(Y_i - \hat{Y}_i)^2}{n-2}$ (with one explanatory variable, divide by $n-p$ when there are $p-1$ explanatory variables).
...
See the Housing Prices^[@iscam, Inv 5.14] example below in section @sec-houses2.
...
Within a residual plot, you should be looking for the same types of things you want in a scatter plot.  [See the residual plots provided in @sec-cat2.]
...
### Intervals for response variable {#sec-infresp}

As in section @sec-mean1inf, we can calculate intervals for both the average response and the individual response.  Now, the intervals depend on how far the explanatory variable is from the mean. If you are trying to predict $Y$ for and $x$ value which is far from $\overline{x},$ then you will be less certain about your prediction. Recall the slope simulation from the [Rossman-Chance applet](https://www.rossmanchance.com/applets/2021/regshuffle/regshuffle.htm).  When we force $\beta_1 = 1,$ we see that the $b_1$ slopes vary nicely around $\beta_1 = 1.$ However, the prediction of $Y$ (height) at 30cm (footlength) is much less variable than the prediction of $Y$ (height) at 45cm (footlength).


```{r fig.cap = "Using the applet, we can see both the variability of b1 (right most histogram) as well as the variability of the predictions of Y (center scatterplot). The predictions are more variable at x values far from the center. https://www.rossmanchance.com/applets/2021/regshuffle/regshuffle.htm", fig.align='center', echo=FALSE}
knitr::include_graphics("figs/predint.png")
```


#### Confidence Interval for average response {-}

Recall, $MSE = \frac{\sum_i(Y_i - \hat{Y}_i)^2}{n-2}$ (with one explanatory variable, divide by $n-p$ when there are $p-1$ explanatory variables).

The standard error of the **average** response value $x_h$ is given by:

$$SE(\hat{Y}_{h}) = \sqrt{MSE \cdot\bigg(\frac{1}{n} + \frac{(x_h - \overline{x})^2}{\sum_{i}(x_i - \overline{x})^2}\bigg)}$$
Which leads to a confidence interval for the average response variable at $x_h$:

$\hat{Y}_h = b_0 + b_1 \cdot x_h$$

$$\hat{Y}_h \pm t^*_{1-\alpha/2, n-2} \cdot \sqrt{MSE \cdot\bigg(\frac{1}{n} + \frac{(x_h - \overline{x})^2}{\sum_{i}(x_i - \overline{x})^2}\bigg)}$$

Note that $t^*_{1-\alpha/2, n-2}$ is the multiplier that comes from the $t_{n-2}$ curve (distribution).  If the goal is to create a 90% CI, then $\alpha = 0.1,$ so $\alpha/2 = 0.05,$ so the multiplier would be found at 0.95: `xqt(0.95, n-2)`.


#### Prediction Interval for individual response {-}


The standard error of the prediction at the value $x_h$ is given by:

$$SE(Y_{h}) = \sqrt{MSE \cdot\bigg(1 + \frac{1}{n} + \frac{(x_h - \overline{x})^2}{\sum_{i}(x_i - \overline{x})^2}\bigg)}$$
Which leads to a prediction interval for an individual response variable at $x_h$:

$\hat{Y}_h = b_0 + b_1 \cdot x_h$$

$$\hat{Y}_h \pm t^*_{1-\alpha/2, n-2} \cdot \sqrt{MSE \cdot\bigg(1 + \frac{1}{n} + \frac{(x_h - \overline{x})^2}{\sum_{i}(x_i - \overline{x})^2}\bigg)}$$
...
When two explanatory variables are highly correlated, odd things can happen with the coefficients. For example, consider the following data of information from homes on sale in Los Angeles in 2010.
...
Think carefully about the following task to predict the price of a home (in $\ln$ units) using the square feet of the home (in $\ln$ units) and the number of bedrooms.  Below are three different models.  For each model, provide the sign (either positive or negative) of the coefficient with a one sentence explanation.

**Model 1:** Is $\beta_1$ positive or negative, why?
$$\ln (\texttt{price}) = \beta_0 + \beta_1 \ln (\texttt{square feet})$$

**Model 2:** Is $\beta_1$ positive or negative, why?
$$\ln (\texttt{price}) = \beta_0 + \beta_1  \texttt{bedrooms}$$

**Model 3:** Is $\beta_1$ positive or negative, why?  Is $\beta_2$ positive or negative, why?
$$\ln (\texttt{price}) = \beta_0 + \beta_1 \ln (\texttt{square feet}) + \beta_2  \texttt{bedrooms}$$



**Model 1:**
$\beta_1$ is positive because the larger the home, the more expensive it is.

**Model 2:**
$\beta_1$ is positive because the more bedrooms the home has, the more expensive it is.

**Model 3:**
$\beta_1$ is positive because, for a fixed number of bedrooms, the larger the home, the more expensive it is.

$\beta_2$ is **negative!!!** because, for a fixed square footage, the more bedrooms actually makes the house wonky and *less* expensive.
...
**Model 2:**
```{r}
lm(log(price) ~ bed, data = LAhomes) |>
  tidy()
```

**Model 3:**
```{r}
lm(log(price) ~ log(sqft) + bed, data = LAhomes) |>
  tidy()
```
...
```{r}
#| echo: false
#| fig.cap: For a given (log) sqft, the more bedrooms, the lower the predicted log(price).

LAhomes |>
  ggplot(aes(x = log(sqft), y = log(price),
             color = as.factor(bed))) + 
  geom_point(size = 0.5) + 
...
It is important to interpret coefficients carefully when the explanatory variables are correlated. Number of `bed`rooms and `log(sqft)` are very correlated, which is why the `bed` coefficient changed from positive to negative when `log(sqft)` was included in the model.

```{r}
#| fig.cap: The two explanatory / predictor variables (log) sqft and bedrooms are highly correlated.

LAhomes |>
  ggplot(aes(y = log(sqft), x = bed)) + 
  geom_point(size = 0.5) + 
  theme_minimal()  
```

However, multicollinearity does not cause trouble with predictions. If the goal is only to predict the price (or log(price)) of the home, correlated variables do not cause trouble.


#### Interaction {-}

> Interaction happens when the relationship between $X_1$ and $Y$ changes for different levels of $X_2$. 

Consider only the homes that have 3 or fewer bedrooms.  We see that the linear model on `log(sqft)` is really different (i.e., different **slope**) for those homes that have 0 or 1 bedrooms as compared to homes that have 2 or 3 bedrooms. The different slopes indicate that there is interaction!
...
Let's check to see if `log(sqft)` interacts with `pool` when predicting `log(price)`!

The way to model interaction is by letting `pool` be a binary variable (1 = yes pool; 0 = no pool).  Then the model becomes:

$$\widehat{\log(price)} = b_0 + b_1 \cdot \log(sqft) + b_2 \cdot pool + b_3 \cdot \log(sqft) \cdot pool$$

Note that the multiplication with pool can happen because pool is binary!

The model above can be separated into two separate models, one for homes that don't have a pool and one for homes that do have a pool:

Pool no: $\widehat{\log(price)} = b_0 + b_1 \cdot \log(sqft) + b_2 \cdot 0 + b_3 \cdot \log(sqft) \cdot 0 = b_0 + b_1 \cdot \log(sqft)$

Pool yes: $\widehat{\log(price)} = b_0 + b_1 \cdot \log(sqft) + b_2 \cdot 1 + b_3 \cdot \log(sqft) \cdot 1 = (b_0 + b_2)+ (b_1 + b_3) \cdot \log(sqft)$

Notice that the two lines have different intercepts and different slopes.  

If we allow for full interaction, the estimates of the coefficients are the following (we model interaction by multiplying the explanatory variables in the `lm()` function):

```{r}
lm(log(price) ~ log(sqft)*pool, data = LAhomes) |>
  tidy()
```

The hypothesis tests for both $\beta_2$ and $\beta_3$ seem to not be discernible. We will dig into that next. The full interaction model can be plotted as:
...
The test on $\beta_3$ asks if it is important that the lines have different slopes (i.e., $H_0: \beta_3 = 0$).  If the lines are parallel, we get the following plot. Notice that it actually doesn't look much different from the full interaction model.

**Important** The test on $\beta_3$ is done *with all the other variables in the model*. That is, when we run the test of $H_0: \beta_3 = 0$, we are assuming that $\beta_2$ is non zero and estimated in the model (i.e, that the intercepts are different for the two models.)
...
The test on $\beta_2$ asks if it is important that the lines have different intercepts (i.e., $H_0: \beta_2 = 0$).  If the lines have the same intercepts, we get the following plot. The plot looks a little bit different from the full interaction model, but forcing the same intercept and allowing for different slopes, does allow for a bit of the line difference that we see in the full interaction plot.

**Important** The test on $\beta_2$ is done *with all the other variables in the model*. That is, when we run the test of $H_0: \beta_2 = 0$, we are assuming that $\beta_3$ is non zero and estimated in the model (i.e, that the slopes are different for the two models.)
...
ggplot(LAhomes, aes(x = log(sqft), y = log(price), 
  ...
  geom_smooth(method = "lm",
              formula = y ~ 0 + x,   # fit per group with zero intercept
              se = FALSE) +
    theme_minimal() 
```
...
At a first glance, we might think that we don't need either $\beta_2$ or $\beta_3$ in the model.  But that is not true!  Instead, we don't need $\beta_2$ **if $\beta_3$ is in the model**.  And we don't need $\beta_3$ **if $\beta_2$ is in the model**.  Which is to say, we need at least one of them.

To keep the model as simple as possible, we decide to remove the interaction term and keep the additive effect of pool. Notice that the p-value on `pool` is now *tiny*!  We keep both `log(sqft)` and `pool` in the model that is predicting `log(price)`.
...
However, the coefficient on `bedrooms` isn't discernible (significant) (that is, the associated p-value is larger than any reasonable level of significance, like $\alpha = 0.05$).  But wait, how is the p-value even calculated?
...
The coefficient on `bedrooms` is not discernible (significant) **given `sqft` is in the model**.  Note that if we don't have `sqft`, then bedrooms acts as a surrogate for `sqft` and it *is* important (and significant!).  In this case, however, `sqft` is a better predictor than `bedrooms`.
...
* ideally the variables in the model are discernible (significant) (remove variables with high p-values)
...
In Chapter [-@sec-regdesc], $R^2,$ the coefficient of determination, was presented.  Not only is $R^2$ the square of the correlation (when there is only one explanatory variable), but it has an additional interpretation that is useful when there are multiple explanatory variables.  $R^2$ measures how much of the original variability in Y is given by the regression line.  

SSE is "sum of squared errors".  So, $SSE(\overline{Y})$ is the amount the response variable varies on its own.  $SSE(\mbox{least-squares})$ is the amount the response variable varies around the regression line (see @sec-slr).
...
$$R^2_{adj} =  1 - \frac{Var(e_i) / (n - p - 1)}{Var(Y_i) / (n - 1)} = 1 - \frac{\sum_{i=1}^n (Y_i - \hat{Y}_i)^2 / (n - p - 1)}{\sum_{i=1}^n (Y_i - \bar{Y})^2 / (n - 1)}$$
...
Consider comparing the following two models.

Model 1: predict `ln_price` using `sqft`, `baths`, and `bedrooms`

Model 2: predict `ln_price` using `sqft` only


```{r}
lm(ln_price ~  sqft + baths + bedrooms, data=house) |>
  glance()
...
### Example: Cat Jumping (Correlation & SLR) {#sec-cat2}

Consider the cat data given in Investigations 5.6 and 5.13 of @iscam.  The idea is to understand cat jumping `velocity` as a function of body characteristics. Note that the correlation $r = -0.496$  between `bodymass` and `velocity`.^[@iscam, Inv 5.6 & 5.13]
...
There are many other variables collected with the cat data:

```{r}
names(cats)
```

Let's consider a model where we are trying to predict the jumping `velocity` using `bodymass`, `musclemass`, and `hindlimb`. It turns out that all three variables are discernible.  

```{r}
lm(velocity ~ bodymass + musclemass + hindlimb,
   data = cats) |>
  tidy()
```

In the multiple regression setting, we can still find confidence and prediction intervals for a specific cat. However, we need to specify values for all three of the explanatory variables of interest.

Find a CI for cats who weigh 4700 grams, with a muscle mass of 60 grams and hindlimb of 27 mm.

Find a PI interval for cats who weigh 4000 grams, with a muscle mass of 80 grams and hindlimb of 30 mm.


Before looking at the confidence and prediction intervals, ask yourself the following questions:

1. Are the two intervals above centered at the same place or at different places?  If different, explain.

2. Are the two intervals above the same width or different widths?  If different, explain.

3. Do the two intervals above rely on the same technical conditions or different technical conditions?  If different, explain.

4. What are the interpretations of the two intervals?

```{r}
lm(velocity ~ bodymass + musclemass + hindlimb,
   data = cats) |>
  augment(newdata = data.frame(bodymass = 4700,
                               musclemass = 60,
                               hindlimb = 27), 
          interval = "confidence")
...
lm(velocity ~ bodymass + musclemass + hindlimb,
   data = cats)  |>
  augment(newdata = data.frame(bodymass = 4000,
                               musclemass = 80,
                               hindlimb = 30), 
          interval = "prediction")
```
...
1. Different:  the first is centered at $b_0 + b_1 \cdot 4700 + b_1 \cdot 60 + b_2 \cdot 27$ and the second is centered at $b_0 + b_1 \cdot 4000 + b_1 \cdot 80 + b_2 \cdot 30.$

2. Different: the first interval will be much more narrow than the second interval.

3. Different:  while both intervals require independence, linearity, and equal variability, the mean confidence interval does not require normality (with large enough sample sizes) and the individual prediction interval always requires normality.

4. Remember, the CI is for the "line", the PI is for the "points."

CI:  We are 95% confident that the true population **average velocity** for cats who weigh 4700 grams, with a muscle mass of 60 grams and hindlimb of 27 mm is between 288.94 and 323.79 (average velocity of cats).

PI: 95% of cats who weigh 4000 grams, with a muscle mass of 80 grams and hindlimb of 30 mm can jump with a velocity between 344.5 and 440.04 (individual cat velocity). 

### Example: Housing Prices (SLR & MLR & Prediction) {#sec-houses2}

Note: the work here is from a different dataset^[@iscam, Inv 5.14], but it has similar questions and analysis as the work in @sec-MLR.
...
```{r}
lm(ln_price ~ sqft + bedrooms, data=house) |> 
  tidy()
lm(ln_price ~ sqft + bedrooms, data=house) |> 
  glance()
```
...
```{r}
lm(ln_price ~ sqft + baths, data=house) |> 
  tidy()
lm(ln_price ~ sqft + baths, data=house) |> 
  glance()
```
...
Although the $R^2$ value went up (44.84% of variability in log price is explained by sqft and bedrooms), the p-value on bedrooms isn't discernible (significant).  The p-value here can be interpreted as a hypothesis test on the slope coefficient given the other variables in the model.

0.353 = P(a slope of -.06827 or more extreme *if sqft is in the model* and there is no relationship between bedrooms and price)

Our output says that once we have sqft in the model, we don't actually need to know anything about the number of bedrooms (even though bedrooms was a discernible (significant) predictor on its own).
...
### Example: 1918-19 Flu and Excess Deaths {#ex:1819flu}

As we are coming to know, measuring the impact of COVID-19 is difficult.  A 2020 NYTimes article compares total deaths in 2020 as compared with 2015-2019 for 8 different regions.^[thanks to Laura Ring Kapitula at Grand Valley State University for this example]
...
Note that in predicting both `excessDeaths` and `magPeak` the most discernible (significant) variable is `daysNonpharm`.  No other variable adds discernibly (significantly) to the model

```{r}
# excessDeaths as the response variable:
flu_1819 |>
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(excessDeaths ~ responseTime + daysNonpharm, data = _) |>
  tidy()

flu_1819 |>
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(excessDeaths ~  daysNonpharm, data = _) |>
  tidy()

flu_1819 |>
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(excessDeaths ~  responseTime, data = _) |>
  tidy()

# magPeak as the response variable:
flu_1819 |>
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(magPeak ~ responseTime + daysNonpharm, data = _) |>
  tidy()

flu_1819 |>
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(log(magPeak) ~ responseTime + daysNonpharm, data = _) |>
  tidy()
```
...
flu_1819 |>
  ...
  filter(place != "St Paul, MN" & place != "Grand Rapids, MI") |>
  lm(ln_magPeak ~ responseTime + daysNonpharm, data = _) |>
  augment() |>
  ggplot() +
  geom_point(aes(x=.fitted, y = .resid)) + 
  geom_hline(yintercept = 0) +
...
> **Findings** Between Feb 6, 2020, and March 12, 2020, 237 patients were enrolled and randomly assigned to a treatment group (158 to remdesivir and 79 to placebo); one patient in the placebo group who withdrew after randomisation was not included in the ITT population. Remdesivir use was not associated with a difference in time to clinical improvement (hazard ratio 1·23 [95% CI 0·87–1·75]). Although not statistically significant (discernible), patients receiving remdesivir had a numerically faster time to clinical improvement than those receiving placebo among patients with symptom duration of 10 days or less (hazard ratio 1·52 [0·95–2·43]). Adverse events were reported in 102 (66%) of 155 remdesivir recipients versus 50 (64%) of 78 placebo recipients. Remdesivir was stopped early because of adverse events in 18 (12%) patients versus four (5%) patients who stopped placebo early.
...
* **not statistically discernible** (significant) !
...
## {{< fa lightbulb >}} Reflection Questions
...
9. What are the three factors that influence the $SE(b_1)$?  (Note:  when something influences $SE(b_1),$ that means the inference is also effected.  If you have a huge $SE(b_1),$ it will be hard to tell if the slope is discernible (significant) because the t value will be small.)
...
## {{< fa balance-scale >}} Ethics Considerations
