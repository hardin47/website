...
Where $MSE = \frac{\sum_i(Y_i - \hat{Y}_i)^2}{n-2}$ (with one explanatory variable, divide by $n-p$ when there ae $p-1$ explanatory variables).
...
Recall, $MSE = \frac{\sum_i(Y_i - \hat{Y}_i)^2}{n-2}$ (with one explanatory variable, divide by $n-p$ when there ae $p-1$ explanatory variables).
...
### Example: Cat Jumping^[@iscam, Inv 5.6 & 5.13] (Correlation & SLR) {#sec-cat2}
...
### Example: Housing Prices^[@iscam, Inv 5.14]  (SLR & MLR & Prediction) {#sec-houses2}
