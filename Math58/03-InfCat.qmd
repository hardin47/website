...
Consider the example from the beginning of the semester on babies choosing the helper toy (instead of the hinderer),  @sec-helper.  Recall that 14 of the 16 babies chose the helper toy.
...
Let's choose $\alpha = 0.01$.  That means that if $p=0.5$, then we should make a Type I error less than 1% of the time.  From the calculations below, we see that the rejection region is $\{ X \geq 14 \}$.  That is, for the researchers to reject the null hypothesis at the $\alpha = 0.01$ discernibility level, they would have needed to see 14, 15, or 16 babies choose the helper (out of 16).
...
Previously (e.g., Gender discrimination example, @sec-randtest) when working with the proportion of success in two separate groups, the proportion of success was subtracted (see also lab 4).  Next week, differences in proportions will be revisited, see @sec-diffprop.  First up, the new statistic of interest will be relative risk, followed by odds ratios.
...
* The *percent change* is defined as (and also known as *efficacy* as seen, for example, in most papers describing COVID-19 vaccines):
\begin{eqnarray*}
(1 - \widehat{RR})*100\% = \frac{\hat{p}_2 - \hat{p}_1}{\hat{p}_2}*100\% = \mbox{percent change from group 2 to group 1}
\end{eqnarray*}

* The CI for $p_1/p_2$ is typically considered discernible (significant) if 1 is not in the interval.  That is because usually the null hypothesis is $H_0: p_1 = p_2$ or equivalently, $H_0: p_1/p_2 = 1$.
...
ggplot(null_RR) + 
  geom_histogram(aes(x = stat), bins = 30, color = "white") + 
    geom_vline(xintercept = obs_RR, color = "red", linewidth = 2) +
  labs(title = "Simulation Distribution of sample relative risks",
       x = "possible values of the sample relative risks")
```
...
ggplot(null_OR) + 
  geom_histogram(aes(x = stat), bins = 90, color = "white") + 
    geom_vline(xintercept = obs_OR, color = "red", linewidth = 2) +
  labs(title = "Simulation Distribution of sample odds ratios",
       x = "possible values of the sample odds ratios")
```
...
## Difference of two proportions {#sec-diffprop}
...
(a) At the 5% discernibility level, the data provide convincing evidence of a real difference in the proportion who are not affected personally between Americans who make less than \$40,000 annually and Americans who make \$40,000 or more annually.^[(a) FALSE (because the interval overlaps zero)]
