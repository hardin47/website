...
library(tidyverse)
nsamp <- 5
npop <- seq(nsamp, 500, 1)
ntheta <- length(npop)
mse.2xbar <- (4*(npop+1)*(npop-1))/(12*nsamp)
mse.2med <- 4*(npop-1)^2 / (4*nsamp)
mse.max <- 2*((npop-1)/nsamp)^2
mse.adjmax <- ((nsamp+1)/nsamp)^2 * ((npop -1)/nsamp)^2 + (((nsamp+1)/nsamp)*(npop - (npop-1)/nsamp) - npop)^2

mse_data <- data.frame(mse = c(mse.2xbar, mse.2med, mse.max, mse.adjmax),
                       estimator = c(rep("twice xbar -1", ntheta), 
                                     rep("twice median -1", ntheta),
                                     rep("max", ntheta),
                                     rep("(n+1)/n * max", ntheta)),
                       samp_size = rep(nsamp, 4*ntheta),
                       true_N = npop)
...
mse_data <- rbind(mse_data, data.frame(mse = c(mse.2xbar, mse.2med, mse.max, mse.adjmax),
                       estimator = c(rep("twice xbar -1", ntheta), 
                                     rep("twice median -1", ntheta),
                                     rep("max", ntheta),
                                     rep("(n+1)/n * max", ntheta)),
...
mse_data %>%
  ggplot(aes(x = true_N, y = mse)) + 
  geom_line(aes(color = estimator, linetype = estimator)) + 
  facet_wrap(~samp_size) + 
  labs(title = "Theoretical MSE for different estimates of the population size, \nfacet by sample size",
       x = "true population size")
```
