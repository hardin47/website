...
% Below derivation from: https://people.csail.mit.edu/rameshvs/content/gmm-em.pdf 
```
...
%Their failure times were recorded as $x_1, \ldots, x_n$. In a separate experiment, $m$ bulbs were tested, but the individual failure times were not recorded. Only the number of bulbs, $r$, that had failed at time $t$ was recorded.
```
...
```{r echo = FALSE, eval = FALSE}
%  Add information on why not plain old ML:
% https://stats.stackexchange.com/questions/326343/why-use-em-algorithm-instead-of-just-plain-old-ml-for-mixture-model
% page 433-434 http://www.rmki.kfki.hu/~banmi/elte/bishop_em.pdf
```
...
faithful %>%
  ...
  geom_histogram() + 
  xlab("Waiting times for eruption of Old Faithful geyser")
```
...
```{r eval = FALSE, echo = FALSE}
% Add a bit on what happens with three or more groups
% http://www.rmki.kfki.hu/~banmi/elte/bishop_em.pdf

See the following applet for a visual representation of how the EM-algorithm converges: \url{http://www.socr.ucla.edu/applets.dir/mixtureem.html}.\\

% Find (or GENERATE!!!) an image that steps through EM finding the correct cluster centers.
```
