...
# @gelman04 says the \propto \tau comes from a uniform on \mu, \log(\sigma), and \tau, but then I think the prior would be $e^{\tau}$, not just plain \tau.
```
...
```{r eval = FALSE, echo = FALSE}
\iffalse
\subsubsection*{Metropolis Algorithm\footnote{Algorithm and example from @gelman04}}
