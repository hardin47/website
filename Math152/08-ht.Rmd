...
Note, the critical value was found using:
```{r}
qchisq(0.99, 20)
```
...
```{r echo = FALSE, out.width = "80%", fig.cap = "Power as a function of mu.  Reading across the rows, the first plot describes the delta1 test; second plot describes the delta2 test; third plot describes the delta3 (two-sided!) test; last plot puts all three together. On each plot, the dotted horizontal line is at 0.05 (the size), the dashed horizontal line is at 1, and the dotted vertical line is at 4 (the null value of mu)."}
#knitr::include_graphics("figs/power.png")

par(mfrow=c(2,2))
plot(seq(3,5,.1), 1 - pnorm((qnorm(.95)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l", col = 6,
	xlab="mu", ylab="pi(mu | delta1)")
abline(h=1,lty=2)
abline(h=.05, lty=3)
abline(v=4,lty=3)

plot(seq(3,5,.1), pnorm((qnorm(.05)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l", col = 5,
	xlab="mu", ylab="pi(mu | delta2)")
abline(h=1,lty=2)
abline(h=.05, lty=3)
abline(v=4,lty=3)

plot(seq(3,5,.1), pnorm((qnorm(.025)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)) + 
	1 - pnorm((qnorm(.975)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l",
	xlab="mu", ylab="pi(mu | delta3)", col = 1)
abline(h=1,lty=2)
abline(h=.05, lty=3)
abline(v=4,lty=3)

plot(seq(3,5,.1), 1 - pnorm((qnorm(.95)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l", xlab="mu", ylab="pi(mu | delta)", col = 6)
points(seq(3,5,.1), pnorm((qnorm(.05)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l", col = 5)
points(seq(3,5,.1), pnorm((qnorm(.025)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)) + 
	1 - pnorm((qnorm(.975)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l", col = 1)
abline(h=1,lty=2)
abline(h=.05, lty=3)
abline(v=4,lty=3)
```
...
Note that we were able to pull out a test statistic which we knew to have a t-distribution.  That was awesome because it allows us find the critical value for a test with a given size.  If we don't know the distribution of the test statistic, we can't figure out the test with a given size.  We won't always be able to immediately identify the LRT as a function of a stat we know (and to solve for it as nicely as we did above with the t-test).


However, for any general LRT (under regularity conditions like the parameter being in the interior of the parameter space and the derivatives of the likelihood existing) the following holds (but the proof is outside the scope of our work) **when $H_0$ is true**:
\begin{eqnarray*}
2 \ln \Lambda(\underline{X}) \stackrel{n \rightarrow \infty}{\rightarrow} \chi^2_\nu
\end{eqnarray*}
where $\nu =$ dim $\Omega_1$ - dim $\Omega_0.$  We'll discuss degrees of freedom a little bit more below, but generally, we won't have time in this class to be able to fully cover the relevant degrees of freedom for the general LRT.
...
Note:  not only does the derivation lead us to the $\chi^2$ test you may have seen before (and its equivalency to the likelihood ratio test), but it also allows us to do (said equivalent) test without using the $\ln$ function.  You might not care so much about computing logs, but certainly it used to be a big deal (many years ago when these tests were derived).
