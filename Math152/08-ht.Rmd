...
```{r echo = FALSE, out.width = "80%", fig.cap = "Power as a function of mu.  Top plot describes the delta1 test; middle plot describes the delta2 test; bottom plot describes the delta3 (two-sided!) test. On each plot, the dotted horizontal line is at 0.05 (the size), the dashed horizontal line is at 1, and the dotted vertical line is at 4 (the null value of mu)."}
#knitr::include_graphics("figs/power.png")

par(mfrow=c(3,1))
plot(seq(3,5,.1), 1 - pnorm((qnorm(.95)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l",
	xlab="mu", ylab="pi(mu | delta1)")
abline(h=1,lty=2)
abline(h=.05, lty=3)
abline(v=4,lty=3)

plot(seq(3,5,.1), pnorm((qnorm(.05)/sqrt(20) + 4 - seq(3,5,.1))* sqrt(20)), type="l",
