...
ggplot(elephants, aes(x=AGE, y=MATINGS)) +
  geom_point() + 
  theme_minimal() + 
  scale_y_continuous(breaks = pretty(c(0, 10), n = 5)) +
  labs(x = "elehant's age",
       y = "number of matings")
...
```{r out.width = "90%", echo = FALSE, fig.cap = "Visualizing Normal vs Poisson Error Regression from @bysh."}
knitr::include_graphics("figs/NormPoisReg1.jpg")
```
...
```{r out.width = "90%", echo = FALSE, fig.cap = "Differences in Normal vs Poisson Error Regression from @bysh."}
knitr::include_graphics("figs/NormPoisReg2.jpg")
```
...
ggplot(elephants, aes(x=MATINGS)) + geom_histogram(bins=30) +
  scale_x_continuous(breaks = seq(min(elephants$MATINGS), 
                                  max(elephants$MATINGS), by = 1)) + 
    labs(x = "number of matings", y = "count") + 
  theme_minimal()
```

Likely, it makes more sense to look at the distribution of the response variable at each value of the explanatory variables.  Because the sample size is limited, we group the age variable into 5 year increments.  Looking at the plot below, again it seems as though, even when conditioning on the explanatory variable, the response is right skewed with variance dependent on the mean.  

The red dots on each plot correspond to the expected count under the Poisson model given by the average number of matings and total count of elephants (in each age group).
...
elephantsGRP |> ggplot(aes(x=age, y=logmate)) + geom_point() + 
  labs(y = "ln of average number of matings") + 
  theme_minimal()
...
#### Dealing with Overdispersion {#sec-overdis}
