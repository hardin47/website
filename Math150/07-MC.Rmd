...
\begin{eqnarray*}
\hat{FDR}(t)&=& \frac{\# \{ p_i \leq t \mbox{ out of the null tests } \}}{ \# \{p_i \leq t\} + (1 \mbox{ if all } p_i > t)}
\end{eqnarray*}
...
\begin{eqnarray*}
\hat{FDR}(p_i) &=& \frac{p_i \cdot m \cdot \pi_0}{i} < \frac{p_i m}{i}
\end{eqnarray*}
where $\pi_0$ is the proportion of tests which are truly null ($m_0/m$).  Consider the adjusted p-values,

\begin{eqnarray*}
\tilde{p}_j = \min [ (m/i) p_i, \tilde{p}_{j+1} ]
\end{eqnarray*}
Reject any hypothesis such that $\tilde{p}_j \leq \delta$ to control the FDR at $\delta$.



Intuition:  suppose $p_{250} < 0.1$.  Show that FDR $< 0.4$. [Because $\tilde{p}_j < 0.1 \cdot 100 / 250$.]  Let $m=100$.

If 250 genes are rejected, 750 are null (presumably).  That is, there are 750 genes with p-values between 0.1 and 1.  Which gives $m_0 \cdot 0.9 = 750 \rightarrow m_0 = 833.33$.

Putting it all together, the number of null hypotheses which were rejected is 833.33 - 750 = 83.33.
\begin{eqnarray*}
...
\begin{eqnarray*}
\hat{q}(p_{(i)}) = \min( \hat{\pi}_0 p_{(i)} \frac{m}{i}, \hat{q}( p_{(i+1)}))
\end{eqnarray*}
...
First note that for both FDR and FWER, the procedure is to {\em control} the errors not to compute or estimate the errors (except in the case of the q-value).
...
\begin{itemize}
\item[case 1]  In the first case, consider $V=R$ such that all significant hypotheses are null.
\begin{eqnarray*}
FDR &=& E\bigg[\frac{V}{R} | R > 0 \bigg] P(R >0)\\
&=& 1 P(R>0) = 1 P(V \geq 1)\\
&=& FWER
\end{eqnarray*}
\item[case 2]  In the second case, consider $V < R$ such that some of the significant hypotheses are null and some are not.  ($V/R < 1$)
\begin{eqnarray*}
FDR &=& E\bigg[\frac{V}{R} | R > 0 \bigg] P(R >0)\\
&=& E\bigg[\frac{V}{R} | R > 0, V\geq 1 \bigg] P(R >0, V \geq 1) + E\bigg[\frac{V}{R} | R > 0, V=0 \bigg] P(R >0, V =0)  \mbox{ (note: } V/R \equiv 0 \mbox{ if } V = 0)\\
