...
Baseball power simulation applet [@iscam]  [http://www.rossmanchance.com/applets/power.html](http://www.rossmanchance.com/applets/power.html)
...
# the function is the numerator of the derivative of PPV wrt n
multn <- function(a,b,n){
(-((1 - a)^n *(-1 + b^n) *log(1 - a)) + (-1 + (1 - a)^n)* b^n  *log(b))
}
...
b <- seq(0.2, 0.99, by = 0.01)
n <- 200

for(i in 1: length(a)){
  for(j in 1:length(b)){
    num <- c(num, multn(a[i], b[j], n))
    alpha <- c(alpha, a[i])
    beta <- c(beta, b[j])
  }
}

data <- data.frame(num, alpha, beta) |>
  mutate(der0 = ifelse(num < 0 , "yes", "no"))

data |>
  ggplot(aes(x = alpha, y = beta, fill = der0)) +
  geom_tile() +
...
Define the estimated false discovery proportion at a cutoff t $(\hat{FDR}(t))$ to be the number of false discoveries at a given cutoff.  Again, to control the FDR at $\delta,$ first order the p-values as $p_1 \leq p_2 \leq \cdots \leq p_m.$
...
```{r echo=FALSE, fig.cap = "All simulations come from the same population.  In the first histogram, the null and alternative hypotheses are the same (i.e., the null hypothesis is true).  In the other three histograms, the null varies (i.e., the null hypothesis is not true).  Each simulation is run with the same true alternative, HA: mu = 47.", out.width='97%'}

set.seed(47)
t0.pvals = replicate(500,t.test(rnorm(20,47,3), mu=47)$p.value)
t1.pvals = replicate(500,t.test(rnorm(20,47,3), mu=40)$p.value)
t2.pvals = replicate(500,t.test(rnorm(20,47,3), mu=48)$p.value)
t3.pvals = replicate(500,t.test(rnorm(20,47,3), mu=50)$p.value)

par(mfrow=c(2,2))
hist(t0.pvals, main="t0 -- H0: mu = 47, HA: mu = 47", xlim=c(0,1))
hist(t1.pvals, main="t1 -- H0: mu = 40, HA: mu = 47", xlim=c(0,1))
hist(t2.pvals, main="t2 -- H0: mu = 48, HA: mu = 47", xlim=c(0,1))
hist(t3.pvals, main="t3 -- H0: mu = 50, HA: mu = 47", xlim=c(0,1))

mtext("p-values under H0 and HA conditions for 500 indep tests", side=3, line=-1.2, outer=T, cex = 1)
```
...
```{r echo=FALSE, fig.cap = "Varying the proportion of null genes (95% and 90%) and the value of the parameter being tested in the null hypothesis. The null hypothesis varies, with the alternative set at HA: mu = 47.", fig.height=20, fig.width=8}
# let g = proportion of null genes

par(mfcol=c(3,2), mar = c(3.5, 2, .5, .7), oma = c(3, 0, 0, 0), 
    mgp = c(1.5, 0.5, 0))
g=0.95
hist(c(t0.pvals[sample(1:500,500*g)], 
       t1.pvals[sample(1:500,500*(1-g))]),
       xlab=paste("t1, prop null =",g), main="pvals")
hist(c(t0.pvals[sample(1:500,500*g)], 
       t2.pvals[sample(1:500,500*(1-g))]),
     xlab=paste("t2, prop null =",g), main="pvals")
hist(c(t0.pvals[sample(1:500,500*g)], 
       t3.pvals[sample(1:500,500*(1-g))]),
     xlab=paste("t3, prop null =",g), main="pvals")
...
```{r echo=FALSE, fig.cap = "Varying the proportion of null genes (80% and 50%) and the value of the parameter being tested in the null hypothesis. The null hypothesis varies, with the alternative set at HA: mu = 47.", fig.height=20, fig.width=8}
# let g = proportion of null genes
...
An important application of multiple comparisons issues comes when deciding whether or not to stop a clinical trial early due to either positive or negative results.  Looking at the data too often will create false positives which can be quite problematic. [@schulz2005]

Consider the following case studies:  

* HIV -- Indinavir was stopped early due to positive results [@hammer1997]
* HVTN 505 was stopped early due to negative results [@huang2015]
...
\begin{eqnarray*}
P( test_1 < \alpha \mbox{ or } test_2 < \alpha \mbox{ or } \ldots \mbox{ or }  test_K < \alpha ) > \alpha
\end{eqnarray*}
...
```{r}
#| fig-cap: "Different $\\alpha^*$ values for three different stopping criteria. Note that Peto does not control the test at an overall $\\alpha=0.05,$ although it is close. [@schulz2005]"
#| fig-width: 8
#| fig-align: 'center'
#| echo: false
knitr::include_graphics("figs/alphaInterim.png")
```
...
The Pocock boundary uses the same alpha value for each test in the interim analysis. Pocock approximated (numerically, no closed form) the probability of a type I error.^[Note, there is only one research question here because we want to know, for example, if the treatment works. We have multiple p-values because we assess the same research question multiple times. But there is only one "type I error" or not.] Which is to say, if the null hypothesis is true, Pocock controls the probability of rejecting at time 1 or time 2 or ... or time $K$.^[Beyond the scope of this class, but the calculation is done with the understanding of the correlation between each test. That is, the information at time 1 is contained in the information at time 2. So we can calculate the covariance between the test statistic at time 1 and the test statistic at time 2.]

Because there is no closed form solution for the Pocock interim analysis cutoff, we use previously calculated tables of cutoffs. 

Advantages:

* simple
* aggressive with respect to stopping early. There is a small expected sample size  (when the effect size is large); that is, strong effects are detected quicly


Disadvantage:

* low power and therefore large maximum sample size (when the effect size is small)
* is based on the test statistic of interest being normally distributed 


#### Peto {-}

Peto is not a formal stopping rule which controls the alpha spending. Indeed, the probability of committing a type I error is larger than $\alpha$ (because $\alpha$ is used at the final look of the data). However, the overall probability of a type I error is not substantially larger than $\alpha$ because of the strict boundary at the earlier stopping times (which are usually set to 0.0001 or so).

Advantages:

* simple to implement.
* conservative early stopping prevents stopping early unless overwhelming evidence

Disadvantages:

* doesn't control the type I error rate at $\alpha$
* result in delay of efficacy or harm

#### O'Brien-Fleming {-}

The O'Brien-Fleming boundary is extremely strict at early stopping times and close to the overall alpha rate at the final analysis time point.  The approximate formula for the cutoff point is:

$$z_k \approx \Phi^{-1}\bigg(1  - \frac{\alpha}{2} \cdot \frac{K}{k}\bigg)$$

for the $k^{th}$ of $K$ looks at the data.

The alpha value for the $k^{th}$ look at the data would then be:

$$\alpha_k \approx 2 \cdot(1 - \Phi(z_k)).$$
...
df_tests_stop <- df_tests |>
  mutate(Pe_alpha = rep(c(0.001, 0.001, 0.001, 0.05), 1000)) |>
  mutate(OF_alpha = rep(c(0.0001, 0.004, 0.019, 0.043), 1000)) |>
  # flag for whether p-value is less than 0.05
  mutate(raw_stop = as.numeric(p_value < 0.05)) |>
  mutate(Bon_stop = as.numeric(p_value < 0.05/4)) |>
  mutate(Po_stop = as.numeric(p_value <  0.018)) |>
  mutate(Pe_stop = as.numeric(p_value < Pe_alpha)) |>
  mutate(OF_stop = as.numeric(p_value < OF_alpha)) |>
  group_by(test) |>
  # first p-value stopped at, if stop == 0 for
  # all weeks this will be the p-value for the first week
  mutate(raw_stop_p_value = dplyr::first(p_value, desc(raw_stop))) |>
  mutate(Bon_stop_p_value = dplyr::first(p_value, desc(Bon_stop))) |>
  mutate(Po_stop_p_value = dplyr::first(p_value, desc(Po_stop))) |>
  mutate(Pe_stop_p_value = dplyr::first(p_value, desc(Pe_stop))) |>
  mutate(OF_stop_p_value = dplyr::first(p_value, desc(OF_stop))) |>
  ungroup()

df_tests_stop |>
  summarise(type1 = mean(raw_stop_p_value < .05))


df_tests_stop |>
  mutate(stop_color = ifelse(raw_stop_p_value < .05, "reject", "don't reject")) |>
  filter(test < 100) |>
  ggplot() +
  geom_line(aes(x = week, y = p_value, group = test, color = stop_color), alpha = .7) +
  geom_hline(yintercept = 0.05, color = "black") +
  scale_color_viridis_d(begin = .9, end = .1) +
  ggtitle("Raw p-value reject: Type 1 ~ 13.1%")
```
...
```{r echo = FALSE}
df_tests_stop |>
  summarise(type1 = mean(Bon_stop_p_value < .05/4))


df_tests_stop |>
  mutate(stop_color = ifelse(Bon_stop_p_value < .05/4, "reject", "don't reject")) |>
  filter(test < 100) |>
  ggplot() +
  geom_line(aes(x = week, y = p_value, group = test, color = stop_color), alpha = .7) +
  geom_hline(yintercept = 0.05, color = "grey") +
  geom_hline(yintercept = 0.05/4, color = "black") +
  scale_color_viridis_d(begin = .9, end = .1) +
  ggtitle("Bonferroni p-value reject: Type 1 ~ 2.7%")
...
```{r echo = FALSE}
df_tests_stop |>
  summarise(type1 = mean(Po_stop_p_value < .018))


df_tests_stop |>
  mutate(stop_color = ifelse(Po_stop_p_value < .018, "reject", "don't reject")) |>
  filter(test < 100) |>
  ggplot() +
  geom_line(aes(x = week, y = p_value, group = test, color = stop_color), alpha = .7) +
  geom_hline(yintercept = 0.05, color = "grey") +
  geom_hline(yintercept = 0.018, color = "black") +
  scale_color_viridis_d(begin = .9, end = .1) +
  ggtitle("Pocock p-value reject: Type 1 ~ 4.8%")
...
df_tests_stop |>
  group_by(test) |>
  mutate(any_stop = max(Pe_stop)) |>
  ungroup() |>
  summarise(type1 = mean(any_stop))
...
df_tests_stop |>
  ...
  mutate(any_stop = max(Pe_stop)) |>
  ungroup()  |>
  mutate(stop_color = ifelse(any_stop == 1, "reject", "don't reject")) |>
  filter(test < 100) |>
  ggplot() +
  geom_line(aes(x = week, y = p_value, group = test, color = stop_color), alpha = .7) +
  geom_hline(yintercept = 0.05, color = "grey") +
  geom_segment(aes(y = 0.001, yend =0.001, x = 0.9, xend = 1.5,), color = "black") +
  geom_segment(aes(y = 0.001, yend =0.001, x = 1.5, xend = 2.5,), color = "black") +
  geom_segment(aes(y = 0.001, yend =0.001, x = 2.5, xend = 3.5,), color = "black") +
  geom_segment(aes(y = 0.05, yend =0.05, x = 3.5, xend = 4.1,), color = "black") +
  scale_color_viridis_d(begin = .9, end = .1) +
  ggtitle("Peto p-value reject: Type 1 ~ 5.4%")
```
...
df_tests_stop |>
  ...
  geom_line(aes(x = week, y = p_value, group = test, color = stop_color), alpha = .7) +
  geom_hline(yintercept = 0.05, color = "grey") +
  geom_segment(aes(y = 0.0001, yend =0.0001, x = 0.9, xend = 1.5,), color = "black") +
  geom_segment(aes(y = 0.004, yend =0.004, x = 1.5, xend = 2.5,), color = "black") +
  geom_segment(aes(y = 0.019, yend =0.019, x = 2.5, xend = 3.5,), color = "black") +
  geom_segment(aes(y = 0.043, yend =0.043, x = 3.5, xend = 4.1,), color = "black") +
  scale_color_viridis_d(begin = .9, end = .1) +
  ggtitle("O'Brien-Fleming p-value reject: Type 1 ~ 4.9%")
