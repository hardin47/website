...
Because there is no closed form soluction for the Pocock interim analysis cutoff, we use previously calculated tables of cutoffs. 
...
Peto is not a formal stopping rule which controls the alpha spending. Indeed, the probability of committing a type I error is larger than $\alpha$ (because $\alpha$ is used at the final look of the data). However, the overall probability of a type I eror is not substantially larger than $\alpha$ because of the strict boundary at the earlier stopping times (which are usually set to 0.0001 or so).
