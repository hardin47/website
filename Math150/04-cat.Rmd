...
##### Agenda {-}
1. Randomization test (binary response)
2. Fisher's Exact test (binary response)
...
## Fisher's Exact Test (section 6.4)
...
```{definition}
**Hyp Geom Prob**  For any $2 \times 2$ table when there are N observations with M total successes , the probability of observing x successes in a sample of size n is:
```
...
## Testing independence of two categorical variables (sections 6.5, 6.6, 6.7)
...
```{example}
The table below show the observed distributions of ABO blood type in three random samples of African Americans living in different locations.
```
...
\begin{eqnarray*}
P(cond1 \ & \ cond2 ) &=& P(cond1) P(cond2)  \ \ \ \ \mbox{ if variables 1 and 2 are independent}\\
...
## Parameter Estimation (section 6.8)
...
\noindent
{\bf Minute Paper} Given the results of the study, do you think we can generalize from the sample to the population?  Explain and make it clear that you know the difference between a sample and a population.
...
##### Agenda {-}
1. Inference on relative risks
2. Inference on odds ratios
