...
```{r echo=FALSE}
elephants <- elephants %>% dplyr::mutate(AGEgroups = ifelse(AGE <=30, "(20,30]", 
                                                            ifelse(AGE <=40, "(30,40]", "(40,55]")))
ggplot(elephants, aes(x=MATINGS)) + geom_histogram(bins=30) + facet_grid(~AGEgroups)
