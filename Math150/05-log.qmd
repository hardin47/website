...
#### Surviving Third-degree Burns {#sec-burnexamp}
...
\end{align}
(see log-linear model below, @sec-altmodels )

#### Alternative strategies for binary outcomes {#sec-altmodels}
...
L(\hat{p}) \geq L(p_0)
\end{align}
always.  [Where $\hat{p}$ is the maximum likelihood estimate for the probability of success. ] The above inequality holds because $\hat{p}$ maximizes the likelihood.
...
See the bird nest example, @sec-birdexamp
...
Data available at: [http://www.biostat.ucsf.edu/vgsm/data/excel/hersdata.xls](http://www.biostat.ucsf.edu/vgsm/data/excel/hersdata.xls)  For now, we will try to predict whether the individuals had a pre-existing medical condition (other than CHD, self reported), `medcond`.  We will use the variables `age`, `weight`, `diabetes` and `drinkany`.
:::


```{r include = FALSE, warning=FALSE, message=FALSE}
HERS <- read.table("./data/HERS.csv", 
...
SAT |> 
  ggplot(aes(x = salary, y = sat)) + 
  geom_point(aes(pch = fracgrp)) + 
  geom_smooth(aes(lty = fracgrp),
              se = FALSE, color = "black", method = "lm", fullrange=TRUE) + 
  theme(legend.position="bottom",
        legend.text=element_text(size=7)) + 
  guides(lty = guide_legend(title.position = "top"),
         pch = guide_legend(title.position = "top")) +
  labs(lty = "Fraction of students taking SAT",
       pch = "Fraction of students taking SAT")
```
...
Consider an example of two baseball players who both played during April and May.
...
You'll notice that Player 2 had a better batting average (BA) in both April and May. However, if the months are combined:
...
...  Player 1 has an overall higher batting average.  Here, Simpson's paradox happens because both players did better in May than in April, but Player 1 had many more at-bats in May (the better month) and Player 2 had many more at-bats in April (the harder month).  As with the SAT example, it would probably be better to report the individual month batting averages instead of the aggregate information.

#### Simpson's Paradox - example 3 {-}
...
library(GGally)
Offering <- read_csv("./data/coins.csv")
...
See here: [https://stats.stackexchange.com/questions/37564/r-code-question-model-selection-based-on-individual-significance-in-regression/37609#37609](https://stats.stackexchange.com/questions/37564/r-code-question-model-selection-based-on-individual-significance-in-regression/37609#37609) for simulation examples where screening/model selection can either include meaningless variables, or leave out important ones.
\end{itemize}
-->
...
Consider the HERS data described in your book (page 30); variable description also given on the book website [http://www.epibiostat.ucsf.edu/biostat/vgsm/data/hersdata.codebook.txt](http://www.epibiostat.ucsf.edu/biostat/vgsm/data/hersdata.codebook.txt)
...
Taken from [https://onlinecourses.science.psu.edu/stat501/node/332](https://onlinecourses.science.psu.edu/stat501/node/332).
...
* For theoretical reasons - that is, the researcher wants to estimate a model based on a known theoretical relationship between the response and predictors.  For example, there may be a known physical relationship (with parameters to estimate) or a differential equation that models state changes.   
...
```{r fig.cap = "Another strategy for model building. Figure taken from [@sleuth].", out.width = "90%", fig.align='center', echo=FALSE}
...
```{r fig.cap = "ROC curve.  Color indicates the probability cutoff used to determine predictions.", out.width = "90%", fig.align='center', echo=FALSE}
knitr::include_graphics("figs/ROCcurve_burn.png")
```
...
## R: Birdnest Example {#sec-birdexamp}
...
nests <- readr::read_csv("./data/C7 Birdnest.csv",
                  na="*")
```
...
Note that the syntax here is slightly different from what we've seen previously.  From the **tidymodels** and **plotROC** packages.  See the full explanation in [Chapter @sec-process].
