...
### Cox PH Regression Analysis {#sec-coxph}
...
### Cox PH Multiple Regression Analysis {#sec-multcoxph}
...
```{r fig.cap = "Table 7.1 from @Dupont.", out.width = "90%", fig.align='center', echo=FALSE}
knitr::include_graphics("figs/table71_CHDHR.png")
```
...
```{r fig.cap = "Table 7.2 from @Dupont.", out.width = "90%", fig.align='center', echo=FALSE}
knitr::include_graphics("figs/table72_CHDHR.png")
```
...
```{r fig.cap = "Table 7.3 from @Dupont.", out.width = "90%", fig.align='center', echo=FALSE}
knitr::include_graphics("figs/table73_CHDHR.png")
```
...
Consider a data frame containing remission survival times of 42 leukemia patients enrolled in a placebo-controlled randomized controlled trial (RCT). The dataset includes information on the time to remission, patient status, sex, white blood cell count (log-transformed), and treatment regimen. [@kleinbaum2012]
...
anderson <- OncoDataSets::LeukemiaSurvival_df
```

The `rx` (treatment) variable does not seem to violate proportional hazards:

```{r}
#| fig-cap: "Assessing the proportional hazards assumption for treatment using a complementary log-log plot: the curves should be approximately parallel if the PH assumption is reasonable."
km_fit_rx <- survfit(Surv(time, status == 1) ~ rx, data = anderson)
km_fit_rx |>
  ggsurvfit::ggsurvfit(type = "cloglog", linetype_aes = TRUE) +
  scale_color_brewer(palette = "Set1")
```

But the `sex` variable absolutely violates proportional hazards!

```{r}
#| fig-cap: "Assessing the proportional hazards assumption for sex using a complementary log-log plot: the curves should be approximately parallel if the PH assumption is reasonable."
km_fit_sex <- survfit(Surv(time, status == 1) ~ sex, data = anderson)
km_fit_sex |>
  ggsurvfit::ggsurvfit(type = "cloglog", linetype_aes = TRUE) +
  scale_color_brewer(palette = "Set1")
```

If we know that the effect of `sex` changes over time (i.e., `sex` and `time` interact in terms of predicting survival), then we might want to fit the model separately for each `sex`:

$$h_i(t) = h_{0,g} e^{\beta_1 \mbox{logWBC}_i + \beta_2 \mbox{rx}_i}$$

That is, the baseline now is able to change for each `sex` (although we still don't actually calculate $h_0(t)$ for either `sex`). The process for calculating coefficients is done using maximum likelihood, as before.  But now the likelihood is split into two separate products (one for male, one for female), and then multiplied together to get the full likelihood.

$$L(\beta) = \prod_{i=1, females}^{r_f} \frac{e^{\beta x_i}}{\sum_{k, females:t_k
\geq t_i} e^{\beta x_k}} \cdot \prod_{i=1, males}^{r_m} \frac{e^{\beta x_i}}{\sum_{k, males:t_k \geq t_i} e^{\beta x_k}}$$


Note that there is no coefficient for the `sex` variable because it isn't in the model any longer!

```{r}
coxph(
  Surv(time, status == 1) ~ logWBC + rx + strata(sex), 
  data = anderson
)
```
...
$$
\begin{align*}
h_i(t) &= h_0(t) e^{\beta_1 \cdot x_{i} + \beta_2 \cdot x_{i} \cdot t}\\
...
The PH test of interest will be: $$H_0: \beta_2 = 0.$$  Note that the test is really asking about whether the time dependency is on $t$  (as opposed, for example, to $\ln(t)$). Testing $\beta_2$ requires that we commit to a particular functional form for the time dependency.

From the example below, it seems as though there might be some time dependency on the effect of sex in the lung dataset (available in the **survival** package). But we've had to think carefully about the functional form of the time dependency, and it still isn't clear that we understand it!

If the test of interest is $HR(t) = e^{\beta_1 x  + \beta_2 x t}:$

```{r}
coxph(
  Surv(time, status) ~ logWBC + rx + sex + tt(sex), 
  data = anderson,
  tt = function(x, t, ...) x * t
)
```

If the test of interest is $HR(t) = e^{\beta_1 x  + \beta_2 x \ln(t)}:$
```{r}
coxph(
  Surv(time, status) ~ logWBC + rx + sex + tt(sex), 
  data = anderson,
...
If the test of interest is $HR(t) = e^{\beta_1 x  + \beta_2 x t^2}:$
...
If the test of interest is $HR(t) = e^{\beta_1 x  + \beta_2 x \sqrt{t}}:$
...
If we agree that the correct model is $HR(t) = e^{\beta_1 x  + \beta_2 x t}$, we might want a CI for $\beta(t) = \beta_1 + \beta_2 t$ 

(Note that $e^{\beta(t)}$ is the HR for the treatment versus control, or here the sex variable. Although it isn't clear if it is hazard of men vs. women or hazard of women vs. men.  `?OncoDataSets::LeukemiaSurvival_df` says that the `sex` variable is "Gender of the patient (numeric, 1 for male, 2 for female)." ... which is **not** how the data are coded!)

$$Var(\beta(t)) = Var(\beta_1) + t^2 \cdot Var(\beta_2) + 2 \cdot t \cdot Cov(\beta_1, \beta_2)$$

$$SE(\beta(t)) = \sqrt{\widehat{Var}(\beta_1) + t^2 \cdot \widehat{Var}(\beta_2) + 2 \cdot t \cdot \widehat{Cov}(\beta_1, \beta_2)}$$

CI for $\beta(t)$: $b_1 + b_2 \cdot t \ \pm \ z* SE(\beta(t))$

Notice that both the statistic (the estimate of the parameter) and the SE are both functions of $t$!

The variance-covariance matrix for the $\beta$ coefficients can be obtained using:
...
$$SE(\beta(t)) = \sqrt{3.0 + t^2 \cdot 0.35 + 2 \cdot t \cdot (-0.98)}$$
...
In R, `cox.zph()` tests whether the effect of $x$ varies in some way with respect to time.  The test is whether $\beta (t) = \beta$  (i.e., is it constant over time?). `cox.zph()` formally assesses the Schoenfeld residuals.

$$H_0: \mbox{The effect of X on the hazard is constant over time}$$
$$H_A: \mbox{The effect of X on the hazard changes over time}$$  
...
6.  Stratify on the variable that violates PH (i.e., fit models separately)
...
There is a package designed to simulate survival data, `simsurv`. It allows for complex models, but there is some difficulty with scenarios with very few events or with binary explanatory variables. Sam Brilleman spoke about the `simsurv` package at useR! 2018: [https://www.youtube.com/watch?v=fJTYsncvpvI](https://www.youtube.com/watch?v=fJTYsncvpvI).
...
The original article is here: [https://www.propublica.org/article/machine-bias-risk-assessments-in-criminal-sentencing](https://www.propublica.org/article/machine-bias-risk-assessments-in-criminal-sentencing)

The data analysis is here: [https://www.propublica.org/article/how-we-analyzed-the-compas-recidivism-algorithm](https://www.propublica.org/article/how-we-analyzed-the-compas-recidivism-algorithm)

The GitHub repo with data and code is here: [https://github.com/propublica/compas-analysis](https://github.com/propublica/compas-analysis)
...
survminer::ggsurvplot(recid.surv, censor=F,
                      title = "Overall")
```

```{r}
ggsurvplot(recid.surv[1],  censor=FALSE,
           title = "Low Only")

ggsurvplot(recid.surv, censor=FALSE, risk.table = TRUE)
```
...
ggsurvplot(survfit(Surv(timefollow,event) ~ score_factor, data=recidKM2), 
           censor=F, title = "No CI")
ggsurvplot(survfit(Surv(timefollow,event) ~ score_factor, data=recidKM2,
                   conf.type="log"), censor=F, conf.int = T, conf.int.style = "step",
           title = "Log CI")
ggsurvplot(survfit(Surv(timefollow,event) ~ score_factor, data=recidKM2,
                   conf.type="log-log"), censor=F, conf.int = T, conf.int.style = "step",
           title = "Log-Log CI")
ggsurvplot(survfit(Surv(timefollow,event) ~ score_factor, data=recidKM2,
                   conf.type="plain"), censor=F, conf.int = T, conf.int.style = "step",
           title = "Plain CI")
