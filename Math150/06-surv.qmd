...
$$SE(\beta(t)) = \widehat{Var}(\beta_1) + t^2 \cdot \widehat{Var}(\beta_2) + 2 \cdot t \cdot \widehat{Cov}(\beta_1, \beta_2)$$

CI for $\beta(t)$: $b_1 + b_2 \cdot t \ \pm \ z* SE(\beta(t))$

Notice here that both the statistic (the estimate of the parameter) and the SE are both functions of $t$!
