...
\begin{align*}
\hat{S}(t)_{KM} &= \prod_{i:t_i \leq t} \frac{n_i - d_i}{n_i}\\
&= \prod_{i:t_i \leq t} p_i\\
...
Recall: $\hat{S}(t)_{KM} = \prod_{k:t_k \leq t} [ (n_k - d_k) / n_k].$

Because $\hat{S}(t)_{KM}$ is just a product of counts, we can "easily" estimate the SE: 

$$
\begin{align*}
s^2_{\hat{S}(t)_{KM}} = \hat{S}(t)_{KM}^2 \sum_{i:t_i \leq t}
...
&= \frac{\sum_{i:t_i \leq t} \frac{d_i}{n_i(n_i - d_i)}}{\bigg[\sum_{i:t_i \leq t} \ln ( (n_i - d_i) / n_i) \bigg]^2}
\end{align*}
$$ 
...
add an example, possible takend from this SO comment, about how transformations can change PH
https://stats.stackexchange.com/questions/379416/log-transform-fixed-ph-in-cox-model-how
-->
