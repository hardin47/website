...
p4 <- ggplot(burndata, aes(x = logarea, y = logitsurv)) + geom_point() + 
  xlab("log area burned") + ylab("logit of prop who survived") + ylim(c(-3,4.2))
...
beta0 <- -0.2
beta1 <- -1
plot(x,sfunc(beta0,beta1),type="l",xlab=paste("beta0=",beta0,"   beta1=", beta1,sep=""),ylab="")
beta0 <- -0.2
beta1 <- 0.4
plot(x,sfunc(beta0,beta1),type="l",xlab=paste("beta0=",beta0,"   beta1=", beta1,sep=""),ylab="")
beta0 <- -0.3
...
```{example}
Consider a simple linear regression model on number of hours studied and exam grade.  Then addclass year to the model.  Note that you would probably have a different slope for each class year in order to model the two variables most effectively.  For simplicity, consider only first year students and seniors.
...
Offering <- Offering %>%
  ...
  mutate(CandyYes = ifelse(Dollars > 1.25, 1, 0))
