...
\begin{enumerate}
  \itemsep0em
  \item Outliers in the X direction: leverage
  \item Outliers in the Y direction: residuals
  \item Influence
  \item VIF
\end{enumerate}
```
...
```{r fig.cap = "The point (a) has very little influence on the model, despite having a large residual.  The point (b) has very little influence in the model because it is consistent with the model given by the other points.  The point (c) has a high residual and high leverage which will impact both the prediction and the coefficients in the model.", echo = FALSE}

set.seed(4747)
x_val <- rnorm(40)
y_val <- x_val*10 + rnorm(40, 0, 5)
point <- rep("none", 40)

data <- data.frame(x_val, y_val, point) %>%
  bind_rows(list(x_val = 0, y_val = 32, point = "(a)")) %>%
  rbind(list(x_val = 1.5, y_val = -27, point = "(c)")) %>%
  rbind(list(x_val = 2.2, y_val = 25, point = "(b)"))

data %>%
  ggplot() + 
  geom_point(aes(x = x_val, y = y_val)) + 
  geom_text_repel(data = filter(data,point !="none"), 
            aes(x = x_val, y = y_val, label = point)) +
  ggtitle("(a) high resid, (b) high leverage, (c) both")
```
...
```{r echo = FALSE, eval = FALSE}
{\em influence / leverage Clicker Questions 62-65}
```
...
```{r echo = FALSE, eval = FALSE}
{\em variance Clicker Questions 66}
```
...
```{r echo = FALSE, eval = FALSE}
{\em diagnostic measures Clicker Questions 66-69}
```
...
```{r echo = FALSE, eval = FALSE}
oz_lm <- births14 %>%
  stats::step(lm(weight ~ 1, data = births14),
              weight ~ mage + mature + weeks + premie + 
                   gained  + sex + habit + whitemom + term, 
              direction = "forward")
```
