...
% See email from Jeff Witmer 8/5/2013, 2/19/2016
[To be honest, this is the wrong analysis of the following data.  The data actually aren't quadratic at all!  In 1983 the NIFC changed how they collected data which had a drastic effect on the number of fires listed but not much of an effect on the acres numbers.  From a phone call with someone at the NIFC: the reporting of small fires in the eastern US is the main thing that changed in 1983 whereas large wildfires in the western US -- the ones that consume many acres -- have been well-tracked both before and after 1983. She told me that she has no problem with me using the Acres numbers below, but she hoped that I would add a footnote about NIFC being uncertain about data collected before 1983...  anyway, the fitting of the line part is still well demonstrated by the idea of including or not including the linear term. \url{http://www.nifc.gov/fireInfo/fireInfo_stats_totalFires.html}]
```


```{r eval = FALSE, echo = FALSE}
fires <- read_delim("http://pages.pomona.edu/~jsh04747/courses/math158/fires.txt",
...
quad_mod_1 <- fires %>%
  lm(Acres ~ I(Year)^2, data = .)

quad_mod_2 <- fires %>%
  lm(Acres ~ Year + I(Year)^2, data = .)
  
quad_mod_3 <- fires %>%
  lm(Acres ~ Year^2, data = .)
...
```{r fig.cap = "The scatterplot looks linear... but the residual plot doesn't!"}
temperature %>%
...
```{r fig.cap = "The scatterplot looks linear... but the residual plot doesn't!", echo = FALSE}
temperature <- temperature %>%
  mutate(sqrt_read = sqrt(Reading + 35),
         ln_temp = log(Temp + 35),
         temp_sq = Temp^2)
...
p1 <- temperature %>%
  ggplot() + 
  geom_point(aes(x = Temp, y = Reading)) + 
  geom_line(aes(x = Temp, y = -4.93 + 1.19*Temp), color = "blue")
...
p3 <- temperature %>%
  ggplot() + 
  geom_point(aes(x = Temp, y = Reading)) + 
  geom_line(aes(x = Temp, y = -14.43 + 0.02*temp_sq), color = "blue")
  
  
p4 <- quad_mod_1 %>% augment() %>%
  ggplot(aes(x = .fitted, y = .resid)) + 
  geom_point() + 
  geom_hline(yintercept = 0)
...
p5 <- temperature %>%
  ggplot() + 
  geom_point(aes(x = Temp, y = Reading)) + 
  geom_line(aes(x = Temp, y = -4.12 + 1.21*Temp - 0.00295*temp_sq), color = "blue")
  
  
p6 <- quad_mod_2 %>% augment() %>%
  ggplot(aes(x = .fitted, y = .resid)) + 
  geom_point() + 
  geom_hline(yintercept = 0)
...
\begin{enumerate}
  ...
  \item F-test
  \item $R^2$ and $R^2_a$
  \item inference on $\beta_k$
  \item prediction \& mean intervals
\end{enumerate}
```
