...
**Akaike's Information Criterion (AIC)**: $$AIC = n\log(SS_\text{Error}) - n \log(n) + 2(p)$$ <br>


**Schwarz's Bayesian Information Criterion (BIC)**: $$BIC = n\log(SS_\text{Error}) - n\log(n) + log(n)\times(p)$$

**Comparison of AIC and BIC**

$$
\begin{eqnarray*} 
&& AIC = \color{blue}{n\log(SS_\text{Error})} - n \log(n) + 2(p) \\
&& BIC = \color{blue}{n\log(SS_\text{Error})} - n\log(n) + \log(n)\times(p) 
\end{eqnarray*}
$$
...
$$\begin{eqnarray*} 
&& AIC = n\log(SS_\text{Error}) - \color{blue}{n \log(n)} + 2(p) \\
&& BIC = n\log(SS_\text{Error}) - \color{blue}{n\log(n)} + \log(n)\times(p) 
\end{eqnarray*}$$

Second Term: Fixed for a given sample size *n*


$$\begin{eqnarray*} && AIC = n\log(SS_\text{Error}) - n\log(n) + \color{blue}{2(p)} \\
&& BIC = n\log(SS_\text{Error}) - n\log(n) + \color{blue}{\log(n)\times(p)} 
\end{eqnarray*}$$
...
$$
R^2 = \frac{SS_{Model}}{SSTO} = 1 - \frac{SSE}{SSTO} = 1 - \frac{686.44}{1913.11} = 0.641
$$
...
$$R^2 = \frac{SS_{Model}}{SSTO} = 1 - \frac{SSE}{SSTO}$$
