...
Let's start with an example, calculating the median arrival delay in the flights table.
...
```{r}
airports |>
  summarize(med_lat = median(lat, na.rm = TRUE))
...
SELECT NTILE(50) OVER (ORDER BY arr_delay) AS med_arr
FROM flights;
...
NTILE(10) OVER w AS 'ntile'
FROM airports
WINDOW w AS (ORDER BY lat);
...
SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY arr_delay) AS med_arr
FROM flights;
```


translating dplyr code: https://r4ds.hadley.nz/databases#sec-sql-expressions

1. windows function / median
2. case_when
3. distinct() / head()
