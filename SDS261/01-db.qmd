...
2. using the **R** package **dpplyr** which translates **dplyr** code into **SQL**.  Recall the data wrangling with which you are already familiar, including `filter()`, `select()`, `group_by()`, `summarize()`, and `arrange()`, all of which live in **dplyr**. See @sec-dplyr-seq.
