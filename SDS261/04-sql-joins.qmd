...
The `movie_info_idx` table does not contain much information about each particular film.  It does, however, have an indicator for the movie ID (given by `movie_id`) as well as the rating (given by `info` where `type_id = 100`).  See @tbl-idx-glance.
